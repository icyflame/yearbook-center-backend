...
function getUserDetails (req, res) {
  User.findOne({ num_id: req.params.id }, '_id num_id username email rollnum name', function (err, user) {
    if (err) {
...
function getAllUsers (req, res) {
  User.find({}, '_id num_id username email rollnum name', function (err, users) {
    if (err) {
      console.log(require('util').inspect(err, { depth: null }));
      res.send(503, err);
    }
